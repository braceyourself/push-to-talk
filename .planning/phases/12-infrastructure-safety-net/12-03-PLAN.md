...
must_haves:
  truths:
    ...
    - "VAD gates what reaches Whisper -- silence and noise are not transcribed"
    - "Transcript segments appear in the TranscriptBuffer with timestamps and source labels"
  ...
  key_links:
    ...
    - from: "continuous_stt.py"
      to: "pasimple"
      via: "records from echo-cancelled source"
      pattern: "device_name.*[Ee]cho"
---
...
<task type="auto">
  ...
  2. In LiveSession.__init__ (or appropriate setup):
     ...
     - Create self._continuous_stt = ContinuousSTT(
         ...
         aec_device_name=config.get("aec_device_name", "Echo Cancellation Source"),
