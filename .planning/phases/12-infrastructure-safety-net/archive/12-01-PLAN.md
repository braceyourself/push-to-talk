...
must_haves:
  truths:
    ...
    - "GPU memory is monitored and the system can detect impending OOM before it happens"
...
<task type="auto">
  <name>Task 2a: VRAM validation spike (go/no-go gate)</name>
  <files></files>
  <action>
  ...
  **IMPORTANT:** If VRAM validation FAILS (doesn't fit at all), stop and report. This is the go/no-go gate.
  ...
  <done>VRAM budget validated with measurements documented. Go/no-go gate passed (or fallback chain documented if marginal).</done>
</task>

<task type="auto">
  <name>Task 2b: PipeWire echo cancellation configuration</name>
  ...
  <done>PipeWire AEC configured and tested -- exact device name recorded in SUMMARY for Plan 03 consumption.</done>
</task>
